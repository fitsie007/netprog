...
public class EmailClient {
    ...
    public String login(String emailAddress, String password){
        ...
        tag = ProjectConstants.TAG_PREFIX + (++index);
        loginCommand = tag +ProjectConstants.LOGIN_COMMAND +emailAddress +" " +password;
        ...
        System.out.println(ProjectConstants.LOGIN_COMMAND  +line +"\n");//authenticated success
        if(line.toLowerCase().contains(ProjectConstants.SUCCESS) || line.toLowerCase().contains(ProjectConstants.COMPLETED) )
            return ProjectConstants.SUCCESS;
        else
            return ProjectConstants.ERROR;
    ...
    public void logout(){
        ...
        tag = ProjectConstants.TAG_PREFIX + (++index);
        logoutCommand = tag +ProjectConstants.LOGOUT_COMMAND;
        ...
        System.out.println(ProjectConstants.LOGOUT_COMMAND  +" "+line +"\n");//logout success
    }
    ...
    public ArrayList<String> getList(){
        ...
        tag = ProjectConstants.TAG_PREFIX +(++index);
        String listCommand = tag +ProjectConstants.LIST_COMMAND + "\"\" \"*\"";
        ...
        System.out.println(ProjectConstants.LIST_COMMAND +line +"\n");//list retrieved successfully
        return mailboxList;
    }
    ...
    public void traverseList(ArrayList<String> mailboxList) {
        Pattern listPattern = Pattern.compile(ProjectConstants.LIST_REGEX);
        String tag, line = null;
...
//                System.out.print("'"+item +"': " +regexMatcher.matches() +"\n");
            if (regexMatcher.find()) {
                ...
                tag = ProjectConstants.TAG_PREFIX + (++index);
                String selectCommand = tag + ProjectConstants.SELECT_COMMAND + mailbox;
                ...
                try {
                    while (!(line = processOutput.readLine()).contains(tag)) {
                        System.out.println(line + "\n");
                        if (line.contains(ProjectConstants.EXISTS_COMMAND)) {
    ...
    /**
     * This method fetches and saves to a file the emails in a certain folder
     * @param numEmails the number of emails in the folder
     * @param mailBox the folder or mailbox from which to fetch the emails
     */
    public void fetchAndSaveEmails(int numEmails, String mailBox) {
        ...
        if (numEmails > 0)
            for (int i = 1; i <= numEmails; i++) {
                tag = ProjectConstants.TAG_PREFIX + (++index);
                String fetchCommand = tag + ProjectConstants.FETCH_COMMAND + i + ProjectConstants.RFC_HEADER;
...
//                System.out.println(i +" of " + numEmails);
                ...
                try {
                    while (!(line = processOutput.readLine()).contains(tag)) {
                        if (line.contains(ProjectConstants.FROM)) {
                            from = line.substring(line.indexOf(ProjectConstants.COLON) + 1, line.length()).trim();
                            if (!from.contains(ProjectConstants.AT_SIGN))
                                from += processOutput.readLine(); //if emailAddress wrapped to next line
                        }
                        if (line.contains(ProjectConstants.SUBJECT))
                            subject = line.substring(line.indexOf(ProjectConstants.COLON) + 1, line.length()).trim();
...
//                        System.out.println(ProjectConstants.ANSI_CLS);
    ...
    /**
     * This method retrieves the message body for a certain email message
    ...
    public String fetchEmail(int index){
        String tag, line;
        tag = ProjectConstants.TAG_PREFIX +(++index);
        String fetchCommand = tag + ProjectConstants.FETCH_COMMAND + index +ProjectConstants.EMAIL_BODY_TAG;
        String message = "";
        output.println(fetchCommand);
        output.flush();
    ...
    public void selectAuthentication(String authenticationType){
        ...
        tag = ProjectConstants.TAG_PREFIX + (++index);
        authenticateCommand = tag +ProjectConstants.AUTHENTICATE_COMMAND + authenticationType;
        ...
        System.out.println(ProjectConstants.AUTHENTICATE_COMMAND  +line +"\n");//success
