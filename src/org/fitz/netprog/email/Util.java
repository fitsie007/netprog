...
public class Util {
    ...
    public static String zeroPad(int num){
        String numStr = num + "";
        String padding="";
        if(numStr.length() < EmailClientConstants.EMAIL_ID_LENGTH)
            padding = new String(new char[EmailClientConstants.EMAIL_ID_LENGTH - numStr.length()]).replace("\0", "0");
    ...
    public static String spacePad(String str){
        String padding="";
        if(str.length() < EmailClientConstants.EMAIL_ID_LENGTH)
            padding = new String(new char[EmailClientConstants.EMAIL_ID_LENGTH - str.length()]).replace("\0", " ");
    ...
    public static String getEmailAddress(String str) {
        ...
        Pattern emailPattern = Pattern.compile(EmailClientConstants.EMAIL_REGEX, Pattern.CASE_INSENSITIVE);
